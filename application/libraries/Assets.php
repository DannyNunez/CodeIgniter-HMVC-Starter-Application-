...
/** Commented out to fix subfolder urls for background images 
...
**/
                    ...
                    if (substr($path, 0, 2) == '//' and substr($new_href, 0, 2) != '//' and substr($new_href, 0, 1) == '/')
                    $new_href = '/' . $new_href;
    ...
    public static function _update_cache_info()
    {
        ...
        foreach (self::$_assets as $key_type => $assets_type) {
            foreach ($assets_type as $key_group => $assets_group) {
                ...
                if (!isset(self::$_cache_info->{$key_type}))
                    self::$_cache_info->{$key_type} = new stdClass;
                if (!isset(self::$_cache_info->{$key_type}->{$key_group}))
    ...
    public static function tag($file = null, $type = null, $echo = true, $attributes = '')
    {
        if ($type === 'css') {
            if (self::$html5)
                $tag = '<link href="' . $file . '"' . $attributes . '>' . PHP_EOL;
            else
                $tag = '<link type="text/css" href="' . $file . '"' . $attributes . ' />' . PHP_EOL;
        }
        elseif ($type === 'js') {
            if (self::$html5)
                $tag = '<script src="' . $file . '"' . $attributes . '></script>' . PHP_EOL;
            else
                $tag = '<script src="' . $file . '" type="text/javascript"' . $attributes . '></script>' . PHP_EOL;
        }
    ...
    private static function _init_cssmin()
    {
        if (self::$minify_css and !self::$freeze and !self::$_cssmin_loaded) {
            ...
            if (defined('SPARKPATH'))
                include(reduce_double_slashes(SPARKPATH . 'assets/' . ASSETS_VERSION . '/libraries/cssmin/cssmin.php'));
            else
                include(reduce_double_slashes(APPPATH . '/third_party/assets/cssmin/cssmin.php'));
    ...
    private static function _init_jsmin()
    {
        if (self::$minify_js and !self::$freeze and !self::$_jsmin_loaded) {
            ...
            // Load
            if (defined('SPARKPATH'))
                include(reduce_double_slashes(SPARKPATH . 'assets/' . ASSETS_VERSION . '/libraries/jsmin/jsmin.php'));
            else
                include(reduce_double_slashes(APPPATH . '/third_party/assets/jsmin/jsmin.php'));
    ...
    private static function _init_jspacker()
    {
        if (self::$pack_js and !self::$freeze and !self::$_jspacker_loaded) {
            // Start benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::init_jspacker()_start");

            // Load
            if (defined('SPARKPATH'))
                include(reduce_double_slashes(SPARKPATH . 'assets/' . ASSETS_VERSION . '/libraries/jspacker/jspacker.php'));
            else
                include(reduce_double_slashes(APPPATH . '/third_party/assets/jspacker/jspacker.php'));
            self::$_jspacker_loaded = true;

            // End benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::init_jspacker()_end");
        }
    }
    ...
    private static function _init_less()
    {
        if (!self::$freeze and !self::$_less_loaded) {
            ...
            // Load LessPHP
            if (defined('SPARKPATH'))
                include(reduce_double_slashes(SPARKPATH . 'assets/' . ASSETS_VERSION . '/libraries/lessc/lessc.php'));
            else
                include(reduce_double_slashes(APPPATH . '/third_party/assets/lessc/lessc.php'));
    ...
    private static function _init_sass()
    {
        if (!self::$freeze and !self::$_sass_loaded) {
            // Start benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::init_sass()_start");

            // Load PHPSass
            if (defined('SPARKPATH'))
                include(reduce_double_slashes(SPARKPATH . 'assets/' . ASSETS_VERSION . '/libraries/phpsass/SassParser.php'));
            else
                include(reduce_double_slashes(APPPATH . '/third_party/assets/phpsass/SassParser.php'));
            ...
            // End benchmark
            if (self::$_enable_benchmark)
                self::$_ci->benchmark->mark("Assets::init_sass()_end");
        }
    }
